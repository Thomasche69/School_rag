...
OLLAMA_SERVER_URL = "http://127.0.0.1:8082"
...
def index_documents(document_chunks):
    """
    Adds document chunks to the session vector store using add_texts.
    """
    try:
...
def find_related_documents(query):
    """
    Finds related documents using similarity search in the session vector store.
    """
    try:
...
def build_prompt_chain(user_query, context_documents):
    ...
    for msg in st.session_state.message_log:
        # Escape literal braces to avoid PromptTemplate format errors
...
def generate_ai_response(prompt_chain, selected_model):
    """
    Sends the prompt chain as structured messages to the Ollama server.
    """
    # Format the prompt chain into actual message objects
    ...
    server_url = OLLAMA_SERVER_URL.strip()  
    
    response = requests.post(
        f"{server_url}/ollama",
        json={"messages": messages_json, "model_name": selected_model}
    )
